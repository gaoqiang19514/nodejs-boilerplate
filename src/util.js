 ...
 * @param {String} imgUrl
 */
exports.getFilename = function getFilename(imgUrl) {
  if (!imgUrl) {
 ...
 * @param {String} imgUrl
 */
exports.getFilePath = function getFilePath(dir, imgUrl) {
  if (!imgUrl) {
 ...
 * @param {String} title
 */
exports.getDir = function getDir(title) {
  if (!title) {
    throw "缺少页面标题，getDir(title)";
  }
  return path.join(DOWNLOAD_PATH, title);
};
 ...
 * @param {String} attr
 */
exports.getCurrPageImgPath = function($, selector, attr, pageUrl) {
  if (!attr) {
    return null;
  }
  const imgPathArr = [];
 ...
 * @param {String} data
 */
exports.createStream = function(data) {
  if (!data) {
...
const WEBSITE_SCHEMA = {
  "www.o26.net": {
    imgsSelector: "#disappear img",
    attr: "src",
    nextPage: ($, currentPageUrl) => {
      let nextPageUrl = $(".page ul")
        .children(".thisclass")
        .next()
        .find("a")
        .attr("href");
      ...
      if (nextPageUrl && nextPageUrl.indexOf("http") === -1) {
        const prefix = getUrlPrefix(currentPageUrl, "/");
    ...
    getPageTitle: $ => {
      let title = $(".article_img_mid h1").text();
  ...
  "www.bobohdy.com": {
    ...
    nextPage: ($, currentUrl) => {
      ...
      nextPageUrl = $(next).find('a').attr('href')
...
exports.getParams = function getParams(url) {
  let urlObj = URL.parse(url, true);
  const website = WEBSITE_SCHEMA[urlObj.host];
  return {
